...
class MenuTestPage(BasePage):
    ...
    def render_sampling(self):
        ...
        name = st.text_input("New Dataset Name", key="sampling_dataset_name")
        if name == "":
            st.error("Please input a dataset name")
        ...
        # )
        method = st.selectbox("Sampling Method", ["PL2N", "Random"], index=0, key="sampling_method")
    ...
    def render_infer_config(self):
        if st.session_state.select_waffle_hub is None:
            return
        st.subheader(f"Inference Config")
        sub_col = st.columns(2, gap="medium")
        with sub_col[0]:
            container = st.container(border=True)
            letter_box = container.checkbox("letter_box", value=True, key="sampling_letter_box")
        with sub_col[1]:
            container = st.container(border=True)
            container.checkbox("half", value=False, key="sampling_half")
        st.slider(
            "Confidence Threshold",
            min_value=0.0,
            max_value=1.0,
            value=0.25,
            key="sampling_confidence_threshold",
        )
        st.slider(
            "IoU Threshold", min_value=0.0, max_value=1.0, value=0.5, key="sampling_iou_threshold"
        )
            ...
            with st.spinner("Sampling..."):
                with TemporaryDirectory() as temp_dir:
                    ...
                    infer_args = {
                        ...
                        "letter_box": st.session_state.sampling_letter_box,
                        "half": st.session_state.sampling_half,
                        "confidence_threshold": st.session_state.sampling_confidence_threshold,
                        "iou_threshold": st.session_state.sampling_iou_threshold,
                        "device": device,
                        "workers": st.session_state.sampling_num_workers,
                    }
                    # inference
                    wh.delete_inference_result(st.session_state.select_waffle_hub)
                    wh.inference(
                        hub=st.session_state.select_waffle_hub,
                        args=infer_args,
                    )
                    # after inference , mv draw dir to result dir
                    draw_dir = image_dir / "draw"
                    io.move_files_to_directory(
                        st.session_state.select_waffle_hub.draw_dir, draw_dir, create_directory=True
                    )

                    st.subheader("Results")
                    image_list = search.get_image_files(directory=result_dir / "draw")
